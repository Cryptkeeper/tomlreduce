...
bool toml_table_can_be_compacted(const toml_table_t* tab, const int depth,
                                 const bool checkStringLength) {
  if (depth < 0) return false;// top-level tables should not be compacted
  ...
  for (int i = 0;; i++) {
    ...
    if (vt != NULL) {
      if (!toml_table_can_be_compacted(vt, depth + 1, checkStringLength))
        return false;
    } else if (va != NULL) {
      if (!toml_array_can_be_compacted(va, false)) return false;
...
bool toml_array_can_be_compacted(const toml_array_t* arr, const int depth) {
  if (depth < 0) return false;// top-level arrays should not be compacted
  ...
  for (int i = 0; i < size; i++) {
    ...
    assert(tab != NULL);
    if (!toml_table_can_be_compacted(tab, depth + 1, false)) return false;
  }
  return true;
}
...
bool toml_array_should_use_newline_between_elements(const toml_array_t* arr,
  ...
  for (int i = 0; i < size; i++) {// check if all tables can be inlined
    const toml_table_t* tab = toml_table_at(arr, i);
    assert(tab != NULL);
    if (!toml_table_can_be_compacted(tab, depth, false)) return true;
  }
